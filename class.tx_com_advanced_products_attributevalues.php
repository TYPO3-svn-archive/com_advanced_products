...
class tx_commerce_attributeValues {
	var $uid = 0;
	var $pid = 0;
	 ...
	 * Constructor
	 */
	function tx_commerce_attributeValues() {
	...
	function createNewPriceCB($PA, $fObj) {
		...
		$content .= '<input type="checkbox" name="data[tx_commerce_attribute_values][' .$PA['row']['uid'] .'][create_new_price]" />';
	...
	function createNewScalePricesCount($PA, $fObj) {
		...
		$content = '<input style="width: 77px;" class="formField1" maxlength="20" type="input" name="data[tx_commerce_attribute_values][' .$PA['row']['uid'] .'][create_new_scale_prices_count]" />';
	...
	function createNewScalePricesSteps($PA, $fObj) {
		...
		$content = '<input style="width: 77px;" class="formField1" maxlength="20"type="input" name="data[tx_commerce_attribute_values][' .$PA['row']['uid'] .'][create_new_scale_prices_steps]" />';	
	...
	function createNewScalePricesStartAmount($PA, $fObj) {
		...
		$content = '<input style="width: 77px;" class="formField1" maxlength="20" type="input" name="data[tx_commerce_attribute_values][' .$PA['row']['uid'] .'][create_new_scale_prices_startamount]" />';
	...
	function deletePriceButton($PA, $fObj)	{
		...
		$pUid = $name[0];
			...
			// build the link code
		$result = '<a href="#" onclick="deleteRecord(\'tx_commerce_attributes_prices\', ' .$pUid .', \'' .$returnUrl .'\');">';
	 ...
	 */
	function attributeValueUid($PA, $fObj) {
		$content.='<input type="hidden" name="'.$PA['itemFormElName'].'" value="'.htmlspecialchars($PA['itemFormElValue']).'">';
 		return $content;
	}
}

if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']["ext/com_advanced_products/class.tx_com_advanced_products_attributevalues.php"])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']["ext/com_advanced_products/class.tx_com_advanced_products_attributevalues.php"]);
}
?>