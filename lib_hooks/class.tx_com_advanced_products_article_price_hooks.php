    ...
    */
	...
	function postpricenet(&$articlePriceData) {
		...
		if((int)$articlePriceData->price_net == 0) {
			...
			//Get the article uid from the database with the uid of the price
			$articleUidRes=$GLOBALS['TYPO3_DB']->exec_SELECTQuery("uid_article",
			'tx_commerce_article_prices',
			'uid = '.$articlePriceData->uid);
			$articleUid = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($articleUidRes);
			$this->article_uid = $articleUid['uid_article'];


			//Get the product data of the article
			//$productData = tx_commerce_belib::getProductOfArticle($this->article_uid);
			$articleData = t3lib_div::makeInstance('tx_commerce_article');
			$articleData->init($articleUid['uid_article']);
			$articleData->load_data();

			$product = $articleData->get_parent_product();


			$this->product_uid = $product->uid;
			...
			if(is_array($article_attributes_values_uids)) {
				...
				foreach ($article_attributes_values_uids as $article_attributes_attributes_uid) {
					if($article_attributes_attributes_uid != 0) {
						...
						if($loaded_prices = $this->load_prices_uid(2)) {
			...
			$attribute_prices_uids_list = implode(',',$this->attribute_prices_uids);

			$attribute_prices_sum_net = 0;

			if($attribute_prices_uids_list) {
				$attributesPriceRes=$GLOBALS['TYPO3_DB']->exec_SELECTQuery("price_net",
				'tx_commerce_attributes_prices',
				'uid IN ('.$attribute_prices_uids_list . ')');
			...
			$this->product_price_uid = $this->load_prices_uid(1);
	...
	function postpricegross(&$articlePriceData) {
		...
		if((int)$articlePriceData->price_gross == 0) {
			...
			//$this->product_uid = $productData['uid'];
			$this->product_uid = $product->uid;
	...
	/**
       ...
       */
	function load_prices_uid($price_table_id){
	...
	/*	function checkForArticlePrice($articleUid) {
	$res=$GLOBALS['TYPO3_DB']->exec_SELECTQuery("price_net,price_gross",
	'tx_commerce_orders',
